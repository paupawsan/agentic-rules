...
def generate_web_config():
    ...
    for plugin_name in plugin_names:
        ...
        # Use explicit config from setup.json (no auto-generation)
        explicit_config = setup_data.get('optional_config', [])
        ...
        plugin_config = {
            ...
            "optional_config": explicit_config,
...
def embed_config_in_html(web_config):
    ...
    version = web_config.get('version', '1.3.0')
    new_html = new_html.replace('{version}', version)
    
    # Update hardcoded version display (e.g., "Version v1.2.0" -> "Version v1.3.0")
